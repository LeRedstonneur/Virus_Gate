...
class Player:
    def __init__(self):
        ...
        self.vitesseMaximale = 126  # La vitesse maximale que le joueur peut atteindre en tombant
    ...
    def update(self, keys: list, screen, obstacles: list, padding: int, pieces: list, checkpoints: list,
               piecesatrouver: list) -> None:
        """Cette méthode est appelée à chaque frame pour actualiser le joueur selon son environnement"""
        self.speed = int(self.vitesse * (1 - self.ralentissement))
        self.jump_height = int(self.hauteur_de_saut * (1 - self.ralentissement / 2))
        self.rectangle = pygame.Rect(-self.posx, -self.posy, self.largeur, self.hauteur)
        ...
        # Lecture des touches
        if keys[pygame.K_SPACE] and (time.time() - self.last_jump_time) > 0.3:  # On ajoute un délai entre les sauts
        ...
        if self.canMove["down"] and not self.surObstacle(obstacles):
            self.vitesseVerticale = max(self.vitesseVerticale - self.gravite, -self.vitesseMaximale)
            # Borne supérieure à la vitesse de chute (assimilable aux frottements de l'air)

        # Si le joueur passe sous la carte, on le remet au dessus
        if self.posy < -2000:
            self.posy = 500
            self.vitesseVerticale = 0
        elif self.posy > 2000:
        ...
        if self.checkpoint < len(checkpoints) and pygame.Rect(checkpoints[self.checkpoint][0],
                                                              checkpoints[self.checkpoint][1], 50, 50).colliderect(
            self.rectangle):
            self.checkpoint += 1
        ...
        if self.checkpoint > 0 and pygame.Rect(checkpoints[self.checkpoint - 1][0], checkpoints[self.checkpoint - 1][1],
                                               50, 50).colliderect(self.rectangle):
            self.pieces_validees = self.pieces
            piecesatrouver.clear()
            piecesatrouver.extend([i for i in pieces])

        # On ajoute le joueur sur l'écran
        pygame.draw.rect(screen, 'BLUE' if self.pause == 0 else "GRAY", (-self.posx + padding, -self.posy, self.largeur, self.hauteur))

        # On dessine la barre de vie sur l'écran
        self.afficherVies(screen)
    ...
    def afficherVies(self, screen) -> None:
        """Affiche en bas à droite de l'écran la barre de vie"""
        text = pygame.font.SysFont("Arial", 36).render(f"{self.vies}/{self.vies_max}", True, (255, 255, 255))
        ...
        # Le rectangle vert en dessous
        position_rect_vert = (x - largeur - 10, y - hauteur - 10)
        rect_vert = pygame.Rect(position_rect_vert, (largeur, hauteur))
        pygame.draw.rect(screen, (0, 255, 0), rect_vert)

        # Le rectangle rouge par dessus, qui le recouvre partiellement ou totalement
        largeur_rect_rouge = largeur * ((self.vies_max - self.vies) / self.vies_max)
        hauteur_rect_rouge = hauteur
    ...
    def afficherPieces(self, screen) -> None:
