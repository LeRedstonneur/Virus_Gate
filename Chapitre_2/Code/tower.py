...
class Tower:
    ...
    def update(self, screen, player: Player, obstacles: list, pause=False):
        if not pause:
            # Rechargement
        ...
        if calculs.distance(self.posx, self.posy, player.posx, player.posy) <= self.range:
            if self.canShoot and not pause:
                self.tirer(self.posx, self.posy, player.posx - (player.largeur / 2), player.posy - (player.hauteur / 2), self.type == "degats_de_zone")
        ...
        for projectile in self.projectiles:
            projectile.update(screen, player, obstacles)
            if projectile.to_delete:
                self.projectiles.remove(projectile)
        if pause:  # Si le jeu est en pause
            self.projectiles = []  # On supprime les projectiles
            pygame.draw.rect(screen, (25, 25, 25), (-self.posx - self.cote / 2 + 5, -self.posy - self.cote / 2 + 5, 40, 40))  # On affiche les tours en gris
    
    def tirer(self, x1: float, y1: float, x2: float, y2: float, explosif: bool) -> None:
...
class Projectile:
    ...
    def update(self, screen, joueur: Player, obstacles: list):
        self.posx += self.vitesseHorizontale
        self.posy += self.vitesseVerticale
        pygame.draw.rect(screen, 'WHITE', (-self.posx - self.cote / 2, -self.posy - self.cote / 2, self.cote, self.cote))
        if self.explosif:
        ...
        for obstacle in obstacles:
            if obstacle.colliderect(-self.posx - self.cote / 2, -self.posy - self.cote / 2, self.cote, self.cote):
                ...
                if self.explosif:
                    # self.draw_circle_alpha(screen, (255, 0, 0, 100), (-self.posx, -self.posy), 50)
                    if calculs.distance(self.posx, self.posy, joueur.posx, joueur.posy) <= 50:
                        joueur.vies -= 1
                        self.to_delete = True
                        joueur.pause = 180
